...
- **Evaluador vs. Participante** — Evaluador (Estudiante/Docente) usa `evaluadorToken` en cookie `httpOnly`. Participante (encuestado externo) usa `participanteToken` en `sessionStorage`. Nunca mezclar su lógica de estado ni de autenticación (ver `JWT_PARTICIPANTE_SECRET`, secreto de firma propio y separado de `evaluadorToken`).
